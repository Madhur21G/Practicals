// Write an algorithm to implement Shortest Remaining Time First scheduling algorithm
...
int getNextProcess(int *arrivalTimes, int *isProcessCompleted, int *bustTimes, int clock, int numberOfProcesses) {
    if (clock == -1) {
        // the first process is the one which arrived first, irrespective of its bust time
        int min = 0;
